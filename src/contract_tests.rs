...
use cosmwasm_std::{from_binary, to_binary, CosmosMsg, DepsMut, Empty, Response, WasmMsg, Coin, Uint128, OwnedDeps};
...
use crate::msg::{DerivativeNft, Metadata};
...
fn mock_dependencies_with_coins() -> OwnedDeps<MockStorage, MockApi, MockQuerier> {
    let coin_balance: &[Coin] = &[Coin {
        denom: String::from("LUNA"),
        amount: Uint128::new(123),
    
    }];
    return mock_dependencies(coin_balance);
...
fn minting_derived() {
    ...
    // mint 2 original tokens - only minter can mint them
    let allowed = mock_info(MINTER, &[]);
    let _ = contract
        .execute(deps.as_mut(), mock_env(), allowed, mint_msg1)
        .unwrap();
    ...
    // derivative nft
    let mint_msg_deriv = ExecuteMsg::Mint(MintMsg::<Extension> {
        ...
        extension: Some(Metadata {
            ...
            animation_url: None,
            youtube_url: None,        
            derivative: Some (DerivativeNft {
                method: String::from("styletransfer"),
                params: None,
                source_ids: vec![String::from("1"),String::from("2")],
            })
        }),
    });

    let allowed = mock_info(&String::from("medusa"), &[]);
    let _ = contract
        .execute(deps.as_mut(), mock_env(), allowed, mint_msg_deriv)
        .unwrap();
    ...
    // derivative nft with one source that we don't own 
    let mint_msg_deriv = ExecuteMsg::Mint(MintMsg::<Extension> {
        ...
        extension: Some(Metadata {
            ...
            derivative: Some (DerivativeNft {
                method: String::from("styletransfer"),
                params: None,
                source_ids: vec![String::from("1"),String::from("3")],
            })
        }),
    });

    let not_allowed = mock_info(&String::from("medusa"), &[]);
    let err = contract
        .execute(deps.as_mut(), mock_env(), not_allowed, mint_msg_deriv).unwrap_err();
        assert_eq!(err, ContractError::Unauthorized {});
